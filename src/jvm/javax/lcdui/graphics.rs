use crate::{app::DRAW_STATE, jvm::jvm_core::{JvmStackValue, HeapObject, JVM}};
...
pub fn handle_virtual_method(
    ...
    match (method_name, descriptor) {
        ...
        ("getStrokeStyle", "()I") => {
            return Ok(Some(JvmStackValue::Int(get_int_field(objectref, jvm, "strokeStyle", 0))));
        }
        ("fillRect", "(IIII)V") => {
        ...
        ("drawRegion", "(Ljavax/microedition/lcdui/Image;IIIIIIII)V") => {
            ...
            draw_region(img_ref, x_src, y_src, width, height, transform, x_dest, y_dest, anchor, jvm);
            Ok(None)
        }
...
fn get_int_arg(args: &[JvmStackValue], index: usize) -> Result<i32, String> {
    match args.get(index) {
        ...
        _ => Err(format!("Graphics method: expected int argument at index {}", index)),
    }
}
...
fn set_int_field(objectref: &JvmStackValue, jvm: &JVM, field_name: &str, value: i32) {
    ...
    if let Some(HeapObject::Instance(obj)) = state.heap.get_mut(heap_idx) {
        obj.fields.insert(field_name.to_string(), JvmStackValue::Int(value));
    }
}
...
fn draw_region(
    ...
    let resource_data = {
        let state = jvm.state.lock();
        let Some(HeapObject::Instance(obj)) = state.heap.get(img_id) else { return; };
    ...
    let Some(data) = resource_data else { return; };
    ...
    // Anchor handling
    if anchor & 1 != 0 { // HCENTER
        real_x -= width / 2;
    } else if anchor & 8 != 0 { // RIGHT
        real_x -= width;
    }

    if anchor & 2 != 0 { // VCENTER
        real_y -= height / 2;
    } else if anchor & 32 != 0 { // BOTTOM
        real_y -= height;
    }
    ...
    if let Some(pixels) = &mut draw_state.pixels {
        ...
        for iy in 0..height {
            for ix in 0..width {
                ...
                if src_px_x < 0 || src_px_x >= rgba.width() as i32 || src_px_y < 0 || src_px_y >= rgba.height() as i32 {
                    continue;
                }

                let px = rgba.get_pixel(src_px_x as u32, src_px_y as u32);
                if px[3] == 0 { continue; } // Fully transparent
                ...
                if offset + 4 <= frame.len() {
                    if px[3] == 255 {
                        ...
                        for i in 0..3 {
                            frame[offset + i] = ((px[i] as f32 * alpha) + (frame[offset + i] as f32 * (1.0 - alpha))) as u8;
                        }
                    }
                }
            }
...
fn fill_triangle(mut x1: i32, mut y1: i32, mut x2: i32, mut y2: i32, mut x3: i32, mut y3: i32, color: [u8; 4]) {
    // Sort vertices by Y
...
fn draw_horizontal_line(mut x1: i32, mut x2: i32, y: i32, dw: i32, dh: i32, frame: &mut [u8], color: [u8; 4]) {
    if y < 0 || y >= dh { return; }
